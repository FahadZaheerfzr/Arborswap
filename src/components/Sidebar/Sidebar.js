...
export default function Sidebar({ fullSidebar, tempfixed, handleTempFixed, activeLink }) {
  ...
  return (
    <div className="w-full  flex flex-col justify-between mb-5">
      <div className="nav-items">
        {activeItem === 'null' && <div className="bg-white dark:bg-black-shade2-background pt-[20%]"></div>}
      ...
      <div className=" flex flex-col items-end mr-7">
        ...
        <div className="socials flex mt-9">
          {socials.map((social) => (
            <div
              key={social.id}
              className="twitter flex items-center justify-center bg-[#F5F1EB] dark:bg-black-shade4-background w-[34px] h-[34px] rounded-md ml-[14px] first:ml-0"
            >
              {social.icon}
            </div>
          ))}
