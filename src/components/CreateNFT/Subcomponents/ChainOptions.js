...
export default function ChainOptions({ item, chainType, setChainType, userFilter }) {
    return (
        <div className={`chain-item flex justify-between items-center mt-4 ${userFilter?"p-2":""}`}>
            ...
            <div className={`flex items-center rounded-full ${userFilter? "w-5 h-5":"w-6 h-6"} justify-center border-[1.5px] border-[#C89211] cursor-pointer`}
                onClick={() => setChainType(item.id)}>
                {chainType === item.id && (
                    <div className={`${userFilter? "w-2 h-2":"w-3 h-3"} rounded-full bg-[#C89211]`} />
                )}
            </div>
        </div>
    )
