...
export default function Preview({ nft, nftType, nftChainType, setActive }) {
    return (
        <div className='preview'>
            ...
            {nft.properties && nft.properties.map((property, index) => (
                <PreviewDetails name={property.name} value={property.value} />
            ))}


            <PreviewHeader heading="NFT Details" />

            {nftType === 1 && nft.copies && (
                <PreviewDetails name="Number of Copies" value={nft.copies} />
            )}
